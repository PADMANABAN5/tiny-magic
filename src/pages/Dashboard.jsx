...
function Dashboard() {
  ...
  const handleSendClick = () => {
    ...
    const simulatedResponse = `This is a simulated response for: "${newPrompt}"`;

    setChatHistory((prev) => [
      ...prev,
      { user: newPrompt, system: simulatedResponse }
    ]);
    setPrompt('');
  };
  ...
  return (
    <div className="d-flex flex-column flex-md-row dashboard-container position-relative">
      <Sidebar />
      <div className="flex-grow-1 dashboard-content position-relative" style={{ padding: '20px', height: '100vh', overflowY: 'auto' }}>
    ...
    {chatHistory.map((item, index) => (
      <div key={index}>
        <div className="d-flex justify-content-end">
          <div className="alert alert-primary mb-1" style={{ maxWidth: '70%' }}>
            <strong>You:</strong> {item.user}
          </div>
        </div>
        <div className="d-flex justify-content-start">
          <div className="alert alert-secondary" style={{ maxWidth: '70%' }}>
