...
import { Card, Badge, Spinner, Alert, OverlayTrigger, Tooltip } from 'react-bootstrap';
...
function Orgadminpods() {
  ...
  useEffect(() => {
    const fetchPods = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_LINK}/orgadmin/pods/${email}`);
        if (response.data.success) {
          const filtered = response.data.data.filter(pod => String(pod.batch_id) === String(batchId));
          setPods(filtered);
        } else {
  ...
  return (
    <div className="d-flex bg-light min-vh-100">
      <Orgadminsidebar />
      <div className="flex-grow-1 d-flex flex-column align-items-end pe-4 pt-4">
<h2
  className="mb-4 fs-3 fw-bold text-dark text-center w-100"
